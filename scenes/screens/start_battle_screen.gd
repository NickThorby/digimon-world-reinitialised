extends Control
## Start Battle Screen — configure opponent teams and launch battles using
## the player's current party from Game.state.
## Side 0 is always the player (read-only, populated from Game.state.party).
## Opponent sides (1+) are fully editable.
...
@onready var _player_count_label: Label = $MarginContainer/VBox/HSplit/LeftPanel/PlayerCountLabel

# --- Right panel (opponent config) ---

@onready var _side_selector: TabBar = $MarginContainer/VBox/HSplit/RightPanel/RightTabs/Opponents/SideSelector
@onready var _opponent_team_list: VBoxContainer = $MarginContainer/VBox/HSplit/RightPanel/RightTabs/Opponents/TeamList
@onready var _add_digimon_button: Button = $MarginContainer/VBox/HSplit/RightPanel/RightTabs/Opponents/AddDigimonButton
@onready var _save_team_button: Button = $MarginContainer/VBox/HSplit/RightPanel/RightTabs/Opponents/TeamButtonRow/SaveTeamButton
@onready var _load_team_button: Button = $MarginContainer/VBox/HSplit/RightPanel/RightTabs/Opponents/TeamButtonRow/LoadTeamButton
...
## Currently selected opponent side (index into config, always >= 1).
var _current_opponent_side: int = 1
...
## Build read-only DigimonSlotPanels for the player's party.
func _build_player_preview() -> void:
	...
	if Game.state == null or Game.state.party.members.is_empty():
		...
		_player_count_label.text = "0 Digimon"
		return
	...
	for i: int in Game.state.party.members.size():
		...
		_player_team_list.add_child(panel)
		panel.setup(i, member)
		panel.set_button_mode(DigimonSlotPanel.ButtonMode.HIDDEN)

	_player_count_label.text = "%d Digimon" % Game.state.party.members.size()


## Construct side 0 config from Game.state (player side, always present).
func _build_player_side_config() -> void:
	if Game.state == null:
		return
	if _config.side_configs.is_empty():
		return
	var player_party: Array[DigimonState] = []
	for member: DigimonState in Game.state.party.members:
		player_party.append(member)
	_config.side_configs[0] = {
...
func _restore_from_battle() -> void:
	...
	if ctx.has("pre_battle_inventory") and Game.state != null:
		...
		var player_bag: Variant = _config.side_configs[0].get("bag") \
			if _config.side_configs.size() > 0 else null
	...
	# Rebuild player side config to reflect updated state
	_build_player_side_config()
...
func _restore_from_picker() -> void:
	...
	var side: int = int(ctx.get("side", 1))
	var editing_idx: int = int(ctx.get("editing_index", -1))

	if side < 1 or side >= _config.side_configs.size():
		return
...
func _on_add_digimon() -> void:
	var max_size: int = _balance.max_party_size if _balance else 6
	if _current_opponent_side < _config.side_configs.size():
		var party: Array = _config.side_configs[_current_opponent_side].get("party", [])
		if party.size() >= max_size:
...
func _on_controller_selected(index: int) -> void:
	if _current_opponent_side < _config.side_configs.size():
		_config.side_configs[_current_opponent_side]["controller"] = \
			index as BattleConfig.ControllerType


func _on_wild_toggled(pressed: bool) -> void:
	if _current_opponent_side < _config.side_configs.size():
		_config.side_configs[_current_opponent_side]["is_wild"] = pressed


func _on_owned_toggled(pressed: bool) -> void:
	if _current_opponent_side < _config.side_configs.size():
		_config.side_configs[_current_opponent_side]["is_owned"] = pressed
...
func _on_launch() -> void:
	...
	# Rebuild player side to ensure latest party state
	_build_player_side_config()
...
func _on_save_team() -> void:
	...
	team.name = "Side %d Team" % (_current_opponent_side + 1)
...
func _open_team_popup(
	mode: TeamSavePopup.PopupMode, team: BuilderTeamState = null
) -> void:
...
func _on_team_loaded(team: BuilderTeamState) -> void:
	...
	for member: DigimonState in team.members:
		if party.size() >= max_size:
			break
		party.append(member)
	_config.side_configs[_current_opponent_side]["party"] = party
	_update_opponent_team_display()
	...
	if loaded_count < total_count:
		...
		_show_validation_message(
			"Loaded '%s' (%d members)." % [team.name, loaded_count]
		)
	_free_team_popup()
...
# --- Side selector & team display ---


func _update_side_selector() -> void:
	_side_selector.clear_tabs()
	for i: int in range(1, _config.side_count):
	...
	if _current_opponent_side >= _config.side_count:
		_current_opponent_side = 1
	# Tab index is side - 1 (since side 0 is player)
	_side_selector.current_tab = _current_opponent_side - 1


func _update_opponent_team_display() -> void:
	for child: Node in _opponent_team_list.get_children():
		child.queue_free()
	...
	for i: int in party.size():
		...
		if state == null:
			continue
		var panel: DigimonSlotPanel = SLOT_PANEL_SCENE.instantiate() as DigimonSlotPanel
		_opponent_team_list.add_child(panel)
		panel.setup(i, state)
...
func _add_empty_placeholder() -> void:
	...
	_opponent_team_list.add_child(placeholder)


func _update_controller_display() -> void:
	if _current_opponent_side >= _config.side_configs.size():
		return
	var controller: int = _config.side_configs[_current_opponent_side].get(
		"controller", BattleConfig.ControllerType.AI
	)
	_controller_option.selected = controller

	var is_wild: bool = _config.side_configs[_current_opponent_side].get(
		"is_wild", false
	)
	_wild_toggle.button_pressed = is_wild

	var is_owned: bool = _config.side_configs[_current_opponent_side].get(
		"is_owned", false
	)
	_owned_toggle.button_pressed = is_owned

	_side_label.text = "Side %d Settings" % (_current_opponent_side + 1)


func _on_slot_edit(index: int) -> void:
	_editing_index = index
	var party: Array = _config.side_configs[_current_opponent_side].get("party", [])
	var existing: DigimonState = party[index] as DigimonState \
		if index < party.size() else null
...
func _get_current_bag() -> BagState:
	if _current_opponent_side >= _config.side_configs.size():
		return BagState.new()
	var cfg: Dictionary = _config.side_configs[_current_opponent_side]
